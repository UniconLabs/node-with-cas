...
{
  winston.info("validate(): begin with ticket validation...");
  ...
  if (pgtURL) {
    winston.info("validate(): pgtURL to use: " + pgtURL);
    query['pgtUrl'] = pgtURL;
  }
...
CAS.prototype.getProxyGrantingTicket = function(pgtIOU, callback)
{
    ...
    // Look up the PGT locally
        ...
        winston.info("getProxyGrantingTicket(): PGT is stored locally. Issuing callback with the received PGT: " + pgt);
...
CAS.prototype.getProxyTicket = function(pgtIOU, targetService, callback) 
{
    ...
    this.getProxyGrantingTicket(pgtIOU, function(err, pgt) {
        ...
        winston.info("getProxyTicket(): Submitting proxy request to the CAS server " + self.hostname + ":" + self.port + self.base_path + "/proxy");
...
CAS.prototype.startProxyServer = function(key, cert, callbackHost, callbackPort, proxyPort) 
{
    ...
    pgtServer.addListener("request", function(req, res) {
        ...
        winston.info("startProxyServer(): proxy server received incoming connection from " + req.url);
...
CAS.prototype.proxiedRequest = function(pgtIOU, options, callback) 
{
    ...
    this.getProxyTicket(pgtIOU, targetService, function(err, pt) {
        ...
        if (path.substring(0,6) == 'https:') {
            serviceObj = https;
            winston.info("proxiedRequest(): using the https package to make the proxy outbound call" );
        } else {
            serviceObj = http;
            winston.info("proxiedRequest(): using the http package to make the proxy outbound call" );
        }
